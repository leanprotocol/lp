  ...
  const handleOpenForm = (plan?: SubscriptionPlan) => {
    if (plan) {
      ...
      setFormData({
        ...
        medicationType: plan.medicationType || "SEMAGLUTIDE",
      });
    } else {
      setEditingPlan(null);
      setFormData({
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="space-y-4">
        {plans.filter(p => (p.medicationType || "SEMAGLUTIDE") === activeTab).length === 0 ? (
          <div className="text-center py-12 bg-gray-50 rounded-xl">
            <p className="text-gray-500">No {activeTab === "MOUNJARO" ? "Mounjaro" : "Semaglutide"} plans yet. Click "Add Plan" to create one.</p>
          </div>
        ) : (
          plans.filter(p => (p.medicationType || "SEMAGLUTIDE") === activeTab).map((plan, index) => (
            <div
              key={plan.id}
              draggable
